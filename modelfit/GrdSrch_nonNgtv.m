...
%%
mode = 'absorb';
eta = [logspace(-1, 3, 80)]; % range of eta
wp = linspace(0, 2, 80);
Mp = [logspace(-1, 3, 80)];  % range of Mp and wp
...
for subj = 1:length(sublist)
    ...
    for modeli = 1:7
        ...
        if modeli <= 2
            ...
            Vq = interp1(eta, [1:numel(eta)], ticks, 'linear');
            xticks(Vq);
            xticklabels(ticks);
        elseif modeli >= 3
            [minVal, Idx] = min(nlls(:));
            ...
            ticks = [.1,1,10,100,1000];
            Vq = interp1(Mp, [1:numel(Mp)], ticks, 'linear');
            yticks(Vq);
            yticklabels(ticks);
            ticks = [0,1,2];
            Vq = interp1(wp, [1:numel(wp)], ticks, 'linear');
            xticks(Vq);
            xticklabels(ticks);
            hold on;
            c = colorbar;
