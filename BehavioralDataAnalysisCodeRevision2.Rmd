...
```{r}
prdct <- read.table(file = '/Users/bs3667/NYU Langone Health Dropbox/Shen Bo/Bo Shen Working files/NoiseProject/Prediction/MeanScaledNoise/Ratio_Model4_MeanScaled_V1_88_sd1_4.5_50V3max83_.txt', header = TRUE)
...
```{r}
prdct <- read.table(file = '/Users/bs3667/NYU Langone Health Dropbox/Shen Bo/Bo Shen Working files/NoiseProject/Prediction/Revision2/Model4_Predict_V11.284661e+02_V22.445383e+02_sd148.4_etavec6.txt', header = TRUE)
plot_dir <- '/Users/bs3667/NYU Langone Health Dropbox/Shen Bo/Bo Shen Working files/NoiseProject/Prediction/Revision2/plot'
...
for (etai in 1:6)
{
  ...
  #if (etai == 1){mycol <- rgb(1, 0, 0)
  #mycol1 <- mycol
  #mycoltrnsp <- rgb(1, 0, 0, .2)}
  #if (v == 'Precise' & tp == 'High'){mycol <- rgb(0, 0, 1)
  #mycoltrnsp <- rgb(0,0,1,.2)}
  #if (etai == 2){mycol <- rgb(1, 191/255, 0)
  #mycol2 <- mycol
  #mycoltrnsp <- rgb(1, 191/255, 0, .2)}
  #if (v == 'Precise' & tp == 'Low'){mycol <- rgb(0, 1, 128/255)
  #mycoltrnsp <- rgb(0, 1, 128/255, .2)}
...
```{r}
Analysis <- '/Users/bs3667/NYU Langone Health Dropbox/Shen Bo/Bo Shen Working files/NoiseProject/ModelfitExtended'
...
fastbads$name <- factor(fastbads$name, levels = c('McFadden','dnLinear','DNM','dnDNM','dnDNMExtened'))
...
{
  N <- sum(grpdcsn$subID == as.character(sublist[si]) & !is.na(grpdcsn$choice)) # number of data points
...
dev.off()
```
## Model post check - Pespective 1: Visualize the trends by Sliding windows along scaled V3 according to the 2x2 design
```{r}
Analysis <- '/Users/bs3667/NYU Langone Health Dropbox/Shen Bo/Bo Shen Working files/NoiseProject/Modelfit/Pooled'
out_dir <- file.path(Analysis,'plot')
for (mdli in 1:4){
  ...
  names(mtmodel)[11:13] <- c('V1scld','V2scld','V3scld')
  names(mtmodel)[32:34] <- c('sdbid1scld','sdbid2scld','sdbid3scld')
  subID <- unique(mtmodel$subID)
  Vagueness <- unique(mtmodel$Vagueness)
  TimePressure <- unique(mtmodel$TimePressure)
  AccID3sim <- c()
  ...
  dev.copy(pdf, file.path(out_dir, sprintf('Model%i_Predict_V3scldtomin_SldWndwWght_ID3.pdf', mdli)), height=4.4, width=7.7)
  dev.off()
}
```
