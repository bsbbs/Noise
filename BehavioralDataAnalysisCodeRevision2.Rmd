...
for (mdli in 1:4)
{
  ...
  # legend('topleft', rep(c('0','.07','.14','.21','.28','.35'),2), lty = 1, ncol = 2,  col = c(cmap1, cmap2))
  dev.copy(pdf, file.path(out_dir,sprintf('CardinalView_Model%i_V3scld_L%.1fH%.1fv3.pdf', mdli, LowestV3, HighestV3)), height=4.4, width=7.7)
  dev.off()
}
```
...
```{r}
Analysis <- '/Users/bs3667/NYU Langone Health Dropbox/Shen Bo/Bo Shen Working files/NoiseProject/Modelfit/ModelfitExtended'
out_dir <- file.path(Analysis,'plot')
if (!dir.exists(out_dir)){dir.create(out_dir)}
fastbads <- read.table(file.path(Analysis,'BestRslts.txt'), header = TRUE, sep = '\t')
demograph <- read.csv('/Users/bs3667/Noise/myData/Demographics.csv', header = TRUE)
...
dev.copy(pdf,file.path(out_dir,sprintf("Model5_vs_Model4.pdf")),height=2.5, width=5)
dev.off()
par(mfrow=c(1,1))
for (mdli in 4)
{
...
dev.copy(pdf,file.path(out_dir,sprintf("Model5_vs_Model4.AIC.pdf")),height=2.5, width=5)
dev.off()
par(mfrow=c(1,1))
for (mdli in 4)
{
...
dev.copy(pdf,file.path(out_dir,sprintf("Model5_vs_Model4.BIC.pdf")),height=2.5, width=5)
dev.off()
...
for (mdli in 1:5)
{
  bars <- fastbads$log.1.delta.[fastbads$modeli == mdli]
  bars <- exp(bars)-1
  hist(bars, 40, probability = TRUE, xlab = expression(delta), main = names[mdli], bty = "n")
  lines(density(bars))
  test <- t.test(bars)
...
for (mdli in 5)
{
  bars <- fastbads$log.1.delta2.[fastbads$modeli == mdli]
  bars <- exp(bars)-1
  hist(bars, 40, probability = TRUE, xlab = expression(delta), main = names[mdli], bty = "n")
  lines(density(bars))
  test <- t.test(bars)
...
dev.copy(pdf,file.path(out_dir,sprintf("delta2.pdf")),height=2.5, width=4.5)
dev.off()
```
