...
for (s in 1:length(subID))
{
  ...
  for (v in Vagueness) # decision noise
  {
    ...
    for (tp in TimePressure) # representation noise
    {
      ...
      CR <- aggregate(choice ~ ID3, data = sectdat, FUN = mean)
      colnames(CR) <- c('ID3','CR')
      Ntrial <- aggregate(trial ~ ID3, data = sectdat, FUN = length)
      colnames(Ntrial) <- c('ID3','Ntrial')
      bid <- aggregate(V3 ~ ID3, data = sectdat, FUN = mean)
      bidscld <- aggregate(V3scld ~ ID3, data = sectdat, FUN = mean)
      sdbid <- aggregate(sdbid3 ~ ID3, data = sectdat, FUN = mean)
      sdbidscld <- aggregate(sdbid3scld ~ ID3, data = sectdat, FUN = mean)
      sectavrg <- merge(CR, bid, by = 'ID3')
      sectavrg <- merge(sectavrg, sdbid, by = 'ID3')
      sectavrg <- merge(sectavrg, bidscld, by = 'ID3')
      sectavrg <- merge(sectavrg, sdbidscld, by = 'ID3')
      sectavrg <- merge(sectavrg, Ntrial, by = 'ID3')
      CR.ID3 <- rbind(CR.ID3, data.frame(subID = subID[s], Vagueness = v, TimePressure = tp, sectavrg))
    }
  }
}
CR.ID3$Vagueness <- as.factor(CR.ID3$Vagueness)
CR.ID3$TimePressure <- as.factor(CR.ID3$TimePressure)
```
...
summary(test <- glm(choice ~ Vagueness + TimePressure + V3:Vagueness:TimePressure, family = "binomial", data = grpdcsn[grpdcsn$choice != 2 & grpdcsn$V3scld >= LowestV3 & grpdcsn$V3scld <= HighestV3,]))

summary(test <- glm(choice ~ Vagueness + TimePressure + V3scld + V3scld:Vagueness + V3scld:TimePressure, family = "binomial", data = grpdcsn[grpdcsn$choice != 2 & grpdcsn$V3scld >= LowestV3 & grpdcsn$V3scld <= HighestV3,]))


summary(test <- glmer(choice ~ TimePressure + V3 + sdbid3 + V3:sdbid3 + (1|subID), family = "binomial", data = grpdcsn[grpdcsn$choice != 2 & grpdcsn$V3scld >= LowestV3 & grpdcsn$V3scld <= HighestV3,]))

summary(test <- glmer(choice ~ TimePressure + V3scld + sdbid3scld + V3scld:sdbid3scld + V3scld:TimePressure + (1|subID), family = "binomial", data = grpdcsn[grpdcsn$choice != 2 & grpdcsn$V3scld >= LowestV3 & grpdcsn$V3scld <= HighestV3,]))

summary(test <- glm(choice ~ TimePressure + V3scld + sdbid3scld + V3scld:sdbid3scld + V3scld:TimePressure, family = "binomial", data = grpdcsn[grpdcsn$choice != 2 & grpdcsn$V3scld >= LowestV3 & grpdcsn$V3scld <= HighestV3,]))


######
summary(aov_model <- aov(CR ~ TimePressure*V3scld*sdbid3scld + Error(subID), data = CR.ID3[CR.ID3$V3scld >= .2 & CR.ID3$V3scld <= .8,]))
summary(lmer_model <- lmer(CR ~ TimePressure*V3scld*sdbid3scld + (1|subID), data = CR.ID3[CR.ID3$V3scld >= .2 & CR.ID3$V3scld <= .8,]))

summary(aov_model <- aov(CR ~ V3scld*TimePressure*Vagueness, data = CR.ID3[CR.ID3$V3scld >= .2 & CR.ID3$V3scld <= .8,]))

summary(lm_model <- lm(CR ~ V3scld*TimePressure*Vagueness, data = CR.ID3[CR.ID3$V3scld >= .2 & CR.ID3$V3scld <= .8,]))

summary(lmer_model <- lmer(CR ~ V3scld*TimePressure*Vagueness + (1|subID), data = CR.ID3[CR.ID3$V3scld >= .2 & CR.ID3$V3scld <= .8,]))

summary(lmer_model <- lmer(CR ~ V3*TimePressure*Vagueness + (1|subID), data = CR.ID3[CR.ID3$V3scld >= .2 & CR.ID3$V3scld <= .8,]))
