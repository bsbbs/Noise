...
V3p = unique([mtconvert.V3scld(mtconvert.Vaguenesscode == 0), mtconvert.sdV3scld(mtconvert.Vaguenesscode == 0)], 'rows'); % precise V3
V3v = unique([mtconvert.V3scld(mtconvert.Vaguenesscode == 1), mtconvert.sdV3scld(mtconvert.Vaguenesscode == 1)], 'rows'); % vague V3
%% Simulation
for modeli = 1:4
    ...
    if ~exist(simdat, 'file')
        ...
        for s = 1:N
            ...
            dat = table('Size', [0 10], 'VariableTypes', {'uint16', 'double', 'double', 'double', 'double', 'double', 'double', 'double', 'double', 'double'},...
                'VariableNames', {'subID', 'V1', 'sdV1', 'V2', 'sdV2', 'V3', 'sdV3', 'Vaguenesscode', 'TimeConstraint', 'chosenItem'});
            indvtask = mt(mt.subID == sublist(s),:);
            Vtrgt = unique([indvtask.V1; indvtask.V2]);
            mintrgt = min(Vtrgt);
            % 'V1', 'V2', 'V3', 'sdV1','sdV2','sdV3'
            V1 = unique_pairs(:,1)*mintrgt;
            sdV1 = unique_pairs(:,2)*mintrgt;
            V2 = unique_pairs(:,2)*mintrgt;
            sdV2 = unique_pairs(:,2)*mintrgt;
            trgt_idx = 1:numel(V1);
            ...
            switch modeli
                ...
                case 4
                    ...
                    wp = fit.wp(subjmask & fit.modeli == modeli);
                    x = [Mp, delta, wp, scl];
                    probs = dnDN(x, dat, 'absorb');
                    name = 'dnDN'; %, cut input, independent';
            end
