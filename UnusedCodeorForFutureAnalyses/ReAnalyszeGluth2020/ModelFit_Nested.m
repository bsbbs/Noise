...
for subj = 1:numel(sublist)
    ...
    for modeli = 1:5
        ...
        parfor i = 1:Npar
            ...
            elseif modeli == 2
                dlmwrite(testfile, [subj, modeli, i, x0(1), NaN, x0(2), xOpt(1), NaN, xOpt(2), fval, output.fsd, exitflag, output.iterations],'delimiter','\t','precision','%.6f','-append');
            elseif modeli == 3
        ...
        if modeli == 1
            new_row = table(subj, modeli, {name}, xOpt(1), NaN, NaN, fval, output.fsd, exitflag, output.iterations, 'VariableNames', Rslts.Properties.VariableNames);
        elseif modeli == 2
            new_row = table(subj, modeli, {name}, xOpt(1), NaN, xOpt(2), fval, output.fsd, exitflag, output.iterations, 'VariableNames', Rslts.Properties.VariableNames);
        elseif modeli == 3
...
Mp = x(1); % change M to be M', absorbing the magnitude of late noise
eta = 1; % after the transformation, the late noise term is standardized as 1
scl = x(2); % scaling parameter on early noise
data = dat(:, {'V1', 'V2', 'V3', 'sdV1','sdV2','sdV3','chosenItem'});
num_samples = 20000;
Ntrl = size(dat,1);
samples = [];
...
eta = 1; % after the transformation, the late noise term is standardized as 1
wp = x(2); % do the same transformation on w
scl = x(3); % scaling parameter on the early noise
data = dat(:, {'V1', 'V2', 'V3', 'sdV1','sdV2','sdV3','chosenItem'});
num_samples = 20000;
Ntrl = size(dat,1);
