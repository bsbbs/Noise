...
axis(2, tck = -0.02, at = c(seq(0,500,100)), padj = 1, cex.axis = 1)
...
dev.copy(pdf,file.path(out_dir,sprintf("Paper_BidVariance_B.pdf")),height=3.4, width=3.1) # height=4.2, width=3.885
dev.off()
```
...
```{r}
mtest1 <- glm(choice ~  V3scld*TimePressure, family = "binomial", data = grpdcsn[grpdcsn$choice == 1 | grpdcsn$choice == 0 & grpdcsn$Vagueness == 'Precise' & grpdcsn$V3scld >= .2 & grpdcsn$V3scld <= .8,])
summary(mtest1)
...
for (ci in condlist) # decision noise
{
  ...
  {plot(v3vec[cut], acc[cut], cex = (Ntrial[cut])/800, col = mycol, 
        xlim = c(-.1, 0.9), ylim = c(62,73), xlab=' ', ylab=' ', 
    ...
    axis(2, tck = -0.02, at = seq(62, 75, 3), padj = 1, cex.axis = 1)
...
dev.copy(pdf, file.path(out_dir,sprintf("Choice~IDV3scldtomin_SldWndwWght_with0.pdf")), height=4.4*1.2, width=4*1.2)
dev.off()
```
...
for (ci in condlist) # decision noise
{
  ...
  {plot(v3vec[cut], acc[cut], cex = (Ntrial[cut])/600, col = mycol,
        xlim = c(LowestV3, .8), ylim = c(62,73), xlab=' ', ylab=' ',
        axes = FALSE, frame.plot = TRUE)
    axis(1, at = c(seq(LowestV3, 1, .2)),  tck = -0.02, padj = -1, cex.axis = 1)
    axis(2, tck = -0.02, at = seq(62, 75, 3), padj = 1, cex.axis = 1)
    title(ylab = '% Correct (V1 & V2)', mgp = c(1.5,0,0), font.lab = 1, cex.lab = 1)
...
#legend(x = .25, y = 73.7, condnames, cex = .8, text.col = condcolors, bty = 'n')
dev.copy(pdf, file.path(out_dir,sprintf("Choice~IDV3scldtomin_SldWndwWght_Segments.pdf")), height=4.4, width=8)
dev.off()
