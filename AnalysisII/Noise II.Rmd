...
barx <- barplot(cbind(means$choice[means$TimePressure=='Low'], means$choice[means$TimePressure=='High'])-0.5, beside = TRUE, names = c('Low','High'), col = c('blue','red','lightblue','pink'), xlab = 'Time Pressure', ylab = '', yaxt = 'n', ylim = c(0,.3)) #ylim = c(-.1,.6))
legend('topright', c('','','Vague','Precise'), col = c('blue','red','lightblue','pink'), bty = 'n', pch = 15, ncol = 2)
axis(2, at = seq(0,.2,.05), labels = seq(50,70,5), tck = -0.03, padj = .7, cex.axis = 1.0, lwd = 1.4)
...
#       points(runif(1,-d1$y[mask],d1$y[mask])*fat1+barx[ci], slct.b1[di]-0.5, pch = 20, cex = .7, col = 8)
#     }
#   }
# }
...
text(mean(xm), .57,'***')
dev.copy(pdf,file.path(out_dir,'LateNoiseReg.pdf'), height=4, width=3.2)
dev.off()
```
