...
model <- glm(
  ...
  data = grpdcsn[(grpdcsn$choice == 1 | grpdcsn$choice == 0) & !is.na(grpdcsn$choice),], # & grpdcsn$V3scld >= .2 & grpdcsn$V3scld <= .8 & !grpdcsn$subID %in% blacklist
  family = binomial(link = "logit"))
summary(model)
...
for (tp in c('High','Low'))
{
  for (vague in c('Precise','Vague'))
  {
    ...
    plot(chrH ~ chrL, data = CHRMsplt[CHRMsplt$TimePressure == tp & CHRMsplt$Vagueness == vague,], pch = 20, main = sprintf('%s %s',tp, vague))
    abline(0,1, lty = 2)
...
dev.copy(pdf,file.path(out_dir,'ChoiceRatioMediumSplit.pdf'), height=7, width=7)
dev.off()
